...
4.  **กลไกประสานงานคู่ขนาน (Parallel Coordination)**:
    ...
    - **กฎ Deadlock Timeout (15 นาที):** หากงานใดๆ ใน `task_locks.json` มีสถานะ `in-progress` นานเกิน 15 นาที `@pm-po` จะถือว่างานนั้น FAILED ทันทีและปลดล็อกเพื่อป้องกันระบบค้าง (Infinite Wait)
...
1.  **บังคับตรวจเช็คสุขภาพข้อมูลด้วย Brain Linter**:
    - ทุกครั้งที่จบงานระบบจะตรวจสอบเอกสารใหม่ใน `second-brain/` **โดยการรันคำสั่งตรวจสอบความสมบูรณ์อัตโนมัติผ่าน IDE Hook เสมอ**:
...
- **อินพุต**: สเปกจาก `[[system_spec]]`, ข้อตกลง API จาก `api_contract.yaml`, และแผนจาก Tech Lead (ตามคำสั่งของ `@pm-po`)
...
- **Lint**: รันการตรวจสอบความสมบูรณ์อัตโนมัติผ่าน IDE Hook ทุกครั้งก่อนจบงาน เพื่อเช็คลิงก์เสีย
...
This project is indexed by GitNexus as **gemini-agent-team-template** (222 symbols, 248 relationships, 2 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
