...
1. ใช้ `view_file` อ่านสเปกระบบจาก `second-brain/10-requirements-spec/system_spec.md` (`[[system_spec]]`) เพื่อทราบขอบเขต API และ Business Logic ที่ต้องตรวจสอบ
...
5. **ห้ามทำการแก้ไขโค้ดด้วยตนเอง** — ให้จดบันทึกช่องโหว่และแนะนำแนวทางป้องกัน (Remediation Steps) โดยละเอียดลงในไฟล์ `second-brain/40-security/security_audit.md` (`[[security_audit]]`) ด้วย `write_to_file`
