...
> 3. **Blind Orchestrator**: คุณต้องทำหน้าที่เป็นผู้คุมงานที่ "ตาบอด" **ห้ามใช้คำสั่ง `view_file` อ่านไฟล์สเปกเทคนิค (เช่น `system_spec.md`, `api_contract.yaml`) หรือซอร์สโค้ดใดๆ ด้วยตัวเองเพื่อตรวจสอบงานเด็ดขาด (เพื่อประหยัด Token)** คุณมีหน้าที่แค่ส่ง Feature Slug ให้ Specialist Agents ไปอ่านไฟล์เหล่านั้นด้วยตนเอง
