...
5. เมื่อ SA ทำงานเสร็จ ให้ส่งเนื้อหาจาก `[[system_spec]]` ไปให้ `@solution-architect` เพื่อวิเคราะห์หาจุดผลกระทบและบันทึกลงไฟล์ `second-brain/20-architecture/architecture_impact.md` (`[[architecture_impact]]`)
...
10. เมื่อ Dev เสร็จ เรียกใช้งาน `@security` สั่งให้ตรวจสอบช่องโหว่ของโค้ดที่ถูกสร้างและรายงานลงใน `[[security_audit]]`
