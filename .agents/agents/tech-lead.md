...
0. **ตรวจสอบและล็อกงาน (Acquire Task Lock)**: ใช้ `view_file` อ่านไฟล์ `second-brain/30-development/features/<slug>/task_locks.json` ดูคีย์ `"tech-lead-plan"`:
   * หากสถานะเป็น `"in-progress"` หรือ `"completed"` ให้ยุติการทำงานทันที
   * หากสถานะเป็น `"idle"` ให้ใช้ `write_to_file` อัปเดตเป็น `"status": "in-progress"`, `"locked_by": "tech-lead"`, `"locked_at": "YYYY-MM-DDTHH:MM:SSZ"` (ใส่ timestamp ปัจจุบัน)
...
4. ใช้ `write_to_file` บันทึกแผนการพัฒนาลงในไฟล์เฉพาะของฟีเจอร์นี้: `second-brain/30-development/features/<slug>/dev-plan.md` เพื่อให้ Backend/Frontend Dev ใช้อ้างอิง
   * **ปลดล็อก**: ใช้ `write_to_file` อัปเดตคีย์ `"tech-lead-plan"` ใน `task_locks.json` เป็น `"status": "completed"`, `"completed_at": "YYYY-MM-DDTHH:MM:SSZ"` (ใส่ timestamp ปัจจุบัน)
